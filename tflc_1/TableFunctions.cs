...
namespace tflc_1
{
    internal class TableFunctions : ParserFunctions
    {
        ...
        public void Fill_Table_Regex(DataGridView table, string[] str, int[] lines, int[] index)
        {
            for (int i = 0; i < str.Length; i++)
            {
                table.Rows.Add(++table_count, path, str[i], Get_Line(lines[i], index[i]), str[i].Length);
            }
        }
